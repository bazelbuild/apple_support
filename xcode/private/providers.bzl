...
def _xcode_version_info_init(
    ...
    platform_struct = apple_common.platform
    platform_type_struct = apple_common.platform_type
    ...
    dotted_ios_minimum_os = apple_common.dotted_version(ios_minimum_os_version)
    dotted_tvos_minimum_os = apple_common.dotted_version(tvos_minimum_os_version)
    dotted_visionos_minimum_os = apple_common.dotted_version(visionos_minimum_os_version)
    dotted_watchos_minimum_os = apple_common.dotted_version(watchos_minimum_os_version)
    dotted_macos_minimum_os = apple_common.dotted_version(macos_minimum_os_version)
    dotted_ios_sdk = apple_common.dotted_version(ios_sdk_version)
    dotted_tvos_sdk = apple_common.dotted_version(tvos_sdk_version)
    dotted_visionos_sdk = apple_common.dotted_version(visionos_sdk_version)
    dotted_watchos_sdk = apple_common.dotted_version(watchos_sdk_version)
    dotted_macos_sdk = apple_common.dotted_version(macos_sdk_version)
    ...
    def _xcode_version(xcode_version):
        ...
        return apple_common.dotted_version(xcode_version)
    ...
    def _minimum_os_for_platform_type(platform_type):
        ...
        if platform_type == platform_type_struct.tvos:
            return dotted_tvos_minimum_os
        if platform_type == platform_type_struct.visionos:
            return dotted_visionos_minimum_os
        if platform_type == platform_type_struct.watchos:
            return dotted_watchos_minimum_os
        if platform_type == platform_type_struct.macos:
    ...
    def _sdk_version_for_platform(platform):
        ...
        if platform.name in (platform_struct.tvos_device.name, platform_struct.tvos_simulator.name):
            return dotted_tvos_sdk
        if platform.name in (platform_struct.visionos_device.name, platform_struct.visionos_simulator.name):
            return dotted_visionos_sdk
        if platform.name in (platform_struct.watchos_device.name, platform_struct.watchos_simulator.name):
            return dotted_watchos_sdk
        if platform.name in (platform_struct.macos.name, platform_struct.catalyst.name):
            # Catalyst builds require usage of the iOS minimum version when
            # building, but require the usage of the macOS SDK to actually do
            # the build. This means that the particular version used for
