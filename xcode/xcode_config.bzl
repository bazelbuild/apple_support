...
_DetectedXcodesInfo = provider(
    doc = "Locally detected Xcode versions used only in diagnostics.",
    fields = {
        "versions": "Full Xcode version strings reported by xcode-locator.",
    },
)

def _detected_xcodes_impl(ctx):
    return [_DetectedXcodesInfo(versions = ctx.attr.versions)]

detected_xcodes = rule(
    implementation = _detected_xcodes_impl,
    attrs = {"versions": attr.string_list()},
    doc = "Diagnostic inventory generated by xcode_autoconf; does not select an Xcode.",
)
...
def _xcode_config_impl(ctx):
    ...
    if _use_available_xcodes(
        ...
        xcode_version_properties = _resolve_explicitly_defined_version(
            ...
            explicit_default_version,
            read_possibly_native_flag(ctx, "xcode_version"),
            ctx.attr.detected_versions[_DetectedXcodesInfo].versions if ctx.attr.detected_versions else [],
            ctx.attr.xcode_selection_hint,
        )
        availability = "UNKNOWN"
...
xcode_config = rule(
    attrs = {
        "xcode_selection_hint": attr.string(
            ...
            doc = """\
Guidance displayed when an explicitly selected Xcode does not match `versions`.
Replaces the default installation and selection instructions. Newlines are
preserved, and an empty string omits the hint.
""",
        ),
...
def _display_xcode_version(version, known_versions = [], detected_versions = []):
    ...
    if "." not in version and not version.isdigit():
        matches = {}
        for full_version in detected_versions + [known_version.xcode_version_properties.xcode_version for known_version in known_versions]:
            if full_version.endswith("." + version):
                matches[_format_xcode_version(full_version)] = True
        if len(matches) == 1:
            return matches.keys()[0]
    return _format_xcode_version(version)

def _format_xcode_version(version):
    # Xcode version strings may append the build number to the release version,
    # e.g. 27.0.27A266a or 27.0.0.27A266a.
    components = version.split(".")
    if len(components) < 3 or components[-1].isdigit():
        return version
    release = components[:-1]
    if not all([component.isdigit() for component in release]):
        return version
    if len(release) == 3 and release[-1] == "0":
        release = release[:-1]
    return "{} (build {})".format(".".join(release), components[-1])
...
def _resolve_xcode_from_local_and_remote(
    ...
    if xcode_version_flag:
        ...
        if remote_version_from_flag and local_version_from_flag:
            ...
            # buildifier: disable=print
            print(("\n\033[1;33mXcode {version} was not found locally.\033[0m\n\n" +
                   "Remote builds can use it; local actions require this Xcode.\n" +
                   "Detected locally: {local_versions}\n\n" +
                   "Install Xcode {version} to run actions locally.\n")
                .format(
                version = _display_xcode_version(remote_version_from_flag.xcode_version_properties.xcode_version),
                local_versions = ", ".join([_display_xcode_version(version.xcode_version_properties.xcode_version) for version in local_versions]) or "none",
            ))
            availability = "REMOTE"
            ...
            _fail_xcode_selection(
                ("\n\033[1;31mXcode {0} was not found.\033[0m\n\n" +
                 ...
                 "Install Xcode {0} and retry.\n").format(
                    _display_xcode_version(xcode_version_flag, local_versions + remote_versions),
        ...
        if flag_version:
            ...
            _fail_xcode_selection(
                ("\n\033[1;31mBazel couldn’t match the selected Xcode to this project’s supported versions:\033[0m\n" +
                 ...
                 "{2}" +
                 "{3}").format(
                    _display_xcode_version(xcode_version_flag, explicit_versions, detected_versions),
                    "\n  ".join([_display_xcode_version(version.xcode_version_properties.xcode_version) for version in explicit_versions]),
                    detected_message,
